...
export default async function TopicDetailPage({
  ...
  const [overview, notes, patterns, mistakes, flashcards, revision] = await Promise.all([
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              <span className={`text-xs font-medium px-2.5 py-0.5 rounded ${difficultyColor[topic.difficulty]}`}>
                {topic.difficulty}
              </span>
              <span className={`text-xs font-medium px-2.5 py-0.5 rounded ${statusColor[topic.status]}`}>
                {topic.status}
              </span>
