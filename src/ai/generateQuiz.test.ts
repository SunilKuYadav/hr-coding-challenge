...
describe('generateQuiz', () => {
  ...
  it('filters out invalid quiz items with missing fields', async () => {
    const data = [
      { question: 'Valid?', options: ['A', 'B'], correctIndex: 0, explanation: 'Yes' },
  ...
  it('filters out quiz items with out-of-bounds correctIndex', async () => {
    const data = [
      { question: 'Bad index?', options: ['A', 'B'], correctIndex: 5, explanation: 'Oops' },
    ];
  ...
  it('returns empty array when generate throws', async () => {
    const client: OllamaClient = {
      ...
      async *generate() { throw new Error('Stream failed'); },
    };
