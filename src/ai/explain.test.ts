...
describe('explainConcept', () => {
  ...
  it('yields streaming explanation chunks', async () => {
    const client = createMockClient(['Hash maps ', 'provide O(1) ', 'lookups.']);
    const chunks: string[] = [];

    for await (const chunk of explainConcept('hash map', 'data structures', client)) {
      chunks.push(chunk);
    }
...
describe('suggestSimilarProblems', () => {
  ...
  it('filters out non-string items from response', async () => {
    const client = createMockClient([JSON.stringify(['Valid', 123, null, 'Also Valid'])]);
    const result = await suggestSimilarProblems(mockProblem, client);
...
describe('generateInterviewPrep', () => {
  ...
  it('yields streaming interview prep chunks', async () => {
    const client = createMockClient(['## Key Questions\n', '1. Brute force approach?']);
    const chunks: string[] = [];
